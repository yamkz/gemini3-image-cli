...
### オブジェクト画像で「登場させたいキャラ/物体/素材」を指定する（--object）

`--object` に画像（ファイル）やフォルダを渡すと、**生成結果に入れたいオブジェクト（キャラクター/物体/素材）の参照画像**としてリクエストに添付されます。

- **フォルダ/単体/混在OK**（`--object` は複数回指定できます）
- **添付順は「通常の入力画像 → オブジェクト画像 → スタイル画像」**で固定されます
- オブジェクト画像がある場合、プロンプトの末尾に自動で以下が追加されます（番号は添付順と一致）：
...
```bash
python run_gemini3_images.py \
  --prompt "ポーチをデザインして、画像として生成して" \
  --object "/path/to/charactor_dir" \
  --candidate_count 3 \
  --single_request
```

※ オブジェクト画像は最大10枚です。11枚以上ある場合は **警告を出して先頭10枚のみ**使用します。
